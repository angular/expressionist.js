...
export class ArrayOfExpression{
  static assert(obj) {
...
export class BoundExpression {
  constructor(expression:Expression, context, wrapper:Function=null){
...
export class Unparser {
  constructor(buffer:ArrayOfString) {
    this.buffer = buffer;
  }

  static unparse(expression:Expression) {
    var buffer = [],
  ...
  writeArgs(args:ArrayOfExpression) {
    this.write('(');
  ...
  visitAccessMember(access:AccessMember) {
    access.object.accept(this);
    this.write(`.${access.name}`);
  }

  visitAccessKeyed(access:AccessKeyed) {
    access.object.accept(this);
    this.write('[');
  ...
  visitCallScope(call:CallScope) {
    this.write(call.name);
    this.writeArgs(call.args);
  }

  visitCallFunction(call:CallFunction) {
    call.func.accept(this);
    this.writeArgs(call.args);
  }

  visitCallMember(call:CallMember) {
    call.object.accept(this);
    this.write(`.${call.name}`);
    this.writeArgs(call.args);
  }

  visitPrefix(prefix:PrefixNot) {
  ...
  visitLiteralPrimitive(literal:LiteralPrimitive) {
    this.write(`${literal.value}`);
  }

  visitLiteralArray(literal:LiteralArray) {
  ...
  visitLiteralObject(literal:LiteralObject) {
  ...
  visitLiteralString(literal:LiteralString) {
...
function ensureFunctionFromMap(obj, name:string){
  var func = obj[name];
